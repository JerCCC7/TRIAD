...
The Hugging Face repository ID will be added after the model repository is published.
...
Once the model repository exists:

```bash
pip install -e '.[hub]'
python scripts/download_checkpoint.py --repo-id YOUR_HF_NAMESPACE/TRIAD
```

`YOUR_HF_NAMESPACE` is a placeholder, not an existing download address.
